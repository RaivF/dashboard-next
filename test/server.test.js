...
describe('server', () => {
  ...
  it('returns health state and manual dashboard statistics', async () => {
    ...
    assert.equal(statistics.manual_applicants_by_date.length, 8)
    ...
    assert.equal(statistics.manual_applicants_by_date.at(-1).date, '2026-06-27')
    assert.equal(statistics.manual_applicants_by_date.at(-1).quantity, 88)
    assert.equal(statistics.manual_funding_by_date.length, 10)
    assert.equal(statistics.manual_funding_by_date[0].categories[0].name, 'Бюджетная основа')
    assert.equal(statistics.manual_funding_by_date[0].categories[0].quantity, 437)
    assert.equal(statistics.manual_funding_by_date[0].categories[1].name, 'Платное обучение')
    assert.equal(statistics.manual_funding_by_date[0].categories[1].quantity, 15)
    assert.equal(statistics.manual_summary.applicationsTotal, 3309)
    assert.equal(statistics.manual_summary.onlineChannels, 448)
    assert.equal(statistics.manual_method[0].name, 'Онлайн')
    assert.equal(statistics.manual_method[0].quantity, 448)
    assert.equal(statistics.manual_method[1].name, 'Очно')
    assert.equal(statistics.manual_method[1].quantity, 2861)
    assert.equal(statistics.manual_top_specialties[0].name, 'Юриспруденция')
    assert.equal(statistics.manual_top_specialties[0].code, '5.40.03.01')
    assert.equal(statistics.manual_top_specialties[0].quantity, 468)
    assert.equal(statistics.manual_bottom_specialties[0].code, '5.39.04.03')
    assert.equal(statistics.manual_bottom_specialties[0].quantity, 1)
    assert.equal(statistics.manual_previous_year_applicants_by_date.length, 8)
    ...
    assert.equal(statistics.manual_previous_year_applicants_by_date[4].date, '2025-06-24')
    assert.equal(statistics.manual_previous_year_applicants_by_date[4].quantity, 170)
    assert.equal(statistics.manual_previous_year_applicants_by_date.at(-1).date, '2025-06-27')
    assert.equal(statistics.manual_previous_year_applicants_by_date.at(-1).quantity, 80)
