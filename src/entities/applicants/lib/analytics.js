...
const ONLINE_METHOD_LABEL = 'Онлайн-каналы'
const METHOD_ORDER = ['Лично', ONLINE_METHOD_LABEL, 'Почта']
const FUNDING_ORDER = [
  'Бюджетная основа',
  'Платное обучение',
  'Целевой прием',
...
const ADMISSION_CAMPAIGN_START_MONTH = 5
const ADMISSION_CAMPAIGN_START_DAY = 20
...
function countUniqueApplicants(items) {
  const keys = new Set()
...
function startOfAdmissionYear(date) {
...
function groupApplicantsByDate(items) {
  const map = new Map()
...
function groupApplicantsByHalfHour(items) {
  const map = new Map()
